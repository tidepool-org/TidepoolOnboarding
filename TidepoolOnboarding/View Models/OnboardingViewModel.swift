...
class OnboardingViewModel: ObservableObject, CGMManagerOnboarding, PumpManagerOnboarding, ServiceOnboarding {
    ...
    func verifyDevice(completion: @escaping (OnboardingError?) -> Void) {
        ...
        tidepoolSecurity.verifyiOSDevice(){ [weak self] result in
            switch result {
            case .success(let valid):
                self?.deviceValid = valid
                completion(nil)
            case .failure(let error):
                guard let error = error as? SecurityError else { completion(.unexpectedError)
                    return
                }
    ...
    func verifyApp(completion: @escaping (OnboardingError?) -> Void) {
        ...
        Task { @MainActor in
            do {
                appValid = try await tidepoolSecurity.verifyApp()
                completion(nil)
            } catch let error as SecurityError {
                log.error("Could not verify app %{public}@", error.localizedDescription.debugDescription)
                completion(error.onboardingError)
            } catch let error {
                log.error("Could not verify app %{public}@", error.localizedDescription.debugDescription)
                completion(.unexpectedState)
            }
        }
    }
...
fileprivate extension TestingScenarioInstance {
    static var thirteenHourTrace: TestingScenarioInstance {
        guard let scenarioURLs = try? FileManager.default.contentsOfDirectory(at: Bundle.main.bundleURL.appendingPathComponent("Scenarios"), includingPropertiesForKeys: nil).filter({ $0.pathExtension == "json" }), let url = scenarioURLs.first(where: { $0.lastPathComponent.contains("13-hour-BG-trace") }), let scenario = try? TestingScenario(source: url).instantiate() else {
            return TestingScenarioInstance(pastGlucoseSamples: [], futureGlucoseSamples: [], pumpEvents: [], carbEntries: [], deviceActions: [], shouldReloadManager: nil)
        }
